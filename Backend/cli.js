...
async function api(method, path, body, token = SESSION.token) {
  ...
  try {
    ...
    return {
      ...
      data: {
        error:
          "Connection refused — is the server running on port " +
...
function printResult(res) {
  if (res.ok) {
    ...
    err(
      `HTTP ${res.status} — ${res.data?.error || res.data?.message || "Unknown error"}`,
    );
...
async function menu(title, options) {
  while (true) {
    ...
    if (SESSION.user) {
      console.log(
        `  ${C.magenta}Logged in as: ${C.bold}${SESSION.user.name}${C.reset} ${C.dim}(${SESSION.user.role})${C.reset}`,
      );
    }
    header(title);
    console.log();
    ...
    if (opt && opt !== null && opt[1]) {
      ...
      if (SESSION.user)
        console.log(
          `  ${C.magenta}${SESSION.user.name}${C.reset} ${C.dim}(${SESSION.user.role})${C.reset}`,
        );
      await opt[1]();
    }
  }
...
async function myProfile() {
  ...
  if (res.ok) {
    ...
    console.log(
      `  ${C.bold}Role:${C.reset}     ${C.yellow}${u.role}${C.reset}`,
    );
    ...
    console.log(
      `  ${C.bold}Joined:${C.reset}   ${new Date(u.createdAt).toLocaleDateString()}`,
    );
...
async function publicOverview() {
  ...
  if (res.ok) {
    ...
    console.log(
      `    Monitoring Locations : ${C.cyan}${stats.totalLocations}${C.reset}`,
    );
    console.log(
      `    Active Industries    : ${C.cyan}${stats.totalIndustries}${C.reset}`,
    );
    console.log(
      `    Regional Offices     : ${C.cyan}${stats.totalRegions}${C.reset}`,
    );
    console.log(
      `    Active Alerts        : ${C.red}${stats.activeAlerts}${C.reset}`,
    );
    console.log(`\n  ${C.bold}Latest Readings:${C.reset}`);
    if (latestReadings.air) {
      const a = latestReadings.air;
      console.log(
        `    🌬  Air @ ${a.location?.name}: AQI=${C.yellow}${a.aqi?.toFixed(1) || "—"}${C.reset}, PM2.5=${a.pm25?.toFixed(1) || "—"} µg/m³`,
      );
    }
    if (latestReadings.water) {
      const w = latestReadings.water;
      console.log(
        `    💧 Water @ ${w.location?.name}: pH=${C.cyan}${w.ph?.toFixed(2) || "—"}${C.reset}, TDS=${w.tds?.toFixed(1) || "—"} mg/L`,
      );
    }
    if (latestReadings.noise) {
      const n = latestReadings.noise;
      console.log(
        `    🔊 Noise @ ${n.location?.name}: Laeq=${C.magenta}${n.laeq?.toFixed(1) || "—"}${C.reset} dB(A)`,
      );
    }
  } else err(res.data?.error);
  await press();
...
async function publicOffices() {
  ...
  if (res.ok) {
    ...
    res.data.forEach((o, i) => {
      console.log(
        `      Industries: ${o._count?.industries}, Locations: ${o._count?.monitoringLocations}`,
      );
    });
  } else err(res.data?.error);
  await press();
...
async function publicAlerts() {
  ...
  if (res.ok && res.data.length > 0) {
    res.data.forEach((a, i) => {
      ...
      console.log(
        `    Location: ${a.location?.name}  |  Type: ${a.type}  |  ${new Date(a.createdAt).toLocaleString()}`,
      );
    });
  ...
  if (res.ok) {
    res.data.forEach((loc) => {
      ...
      if (r) {
        if (type === "air")
          console.log(
            `    AQI: ${C.yellow}${r.aqi?.toFixed(1) || "—"}${C.reset} | PM2.5: ${r.pm25?.toFixed(1)} | PM10: ${r.pm10?.toFixed(1)} | NO2: ${r.no2?.toFixed(1)}`,
          );
        if (type === "water")
          console.log(
            `    pH: ${C.cyan}${r.ph?.toFixed(2) || "—"}${C.reset} | TDS: ${r.tds?.toFixed(1)} | Turbidity: ${r.turbidity?.toFixed(2)} | DO: ${r.dissolvedOxygen?.toFixed(2)}`,
          );
        if (type === "noise")
          console.log(
            `    Laeq: ${C.magenta}${r.laeq?.toFixed(1) || "—"}${C.reset} dB(A) | Lmax: ${r.lmax?.toFixed(1)} | Lmin: ${r.lmin?.toFixed(1)}`,
          );
        console.log(
          `    ${C.dim}${new Date(r.timestamp).toLocaleString()}${C.reset}`,
        );
...
async function publicTrend() {
  ...
  if (!locRes.ok) {
    err("Need to be logged in to list locations");
    await press();
    return;
  }
  printTable(locRes.data, [
    ["name", "Name"],
    ["type", "Type"],
  ]);
  ...
  const res = await api(
    "GET",
    `/public/trend/${type}/${loc.id}?days=${days}`,
    null,
    null,
  );
  if (res.ok) {
    ...
    res.data.slice(-10).forEach((d) => {
      ...
      if (type === "air")
        console.log(
          `  ${C.dim}${ts}${C.reset}  AQI: ${C.yellow}${d.aqi?.toFixed(1) || "—"}${C.reset}  PM2.5: ${d.pm25?.toFixed(1)}`,
        );
      if (type === "water")
        console.log(
          `  ${C.dim}${ts}${C.reset}  pH: ${C.cyan}${d.ph?.toFixed(2) || "—"}${C.reset}  TDS: ${d.tds?.toFixed(1)}`,
        );
      if (type === "noise")
        console.log(
          `  ${C.dim}${ts}${C.reset}  Laeq: ${C.magenta}${d.laeq?.toFixed(1) || "—"}${C.reset} dB(A)`,
        );
    });
...
async function viewData(type) {
  ...
  const limit =
    (await ask("  How many records? (default 10): ")).trim() || "10";
  const filterLoc = await ask(
    "  Filter by location ID? (leave blank for all): ",
  );
  ...
  if (res.ok) {
    ...
    res.data.forEach((d) => {
      ...
      if (type === "air")
        console.log(
          `    AQI: ${C.yellow}${d.aqi?.toFixed(1) || "—"}${C.reset} | PM2.5: ${d.pm25?.toFixed(1)} | PM10: ${d.pm10?.toFixed(1)} | NO2: ${d.no2?.toFixed(1)} | SO2: ${d.so2?.toFixed(1)}`,
        );
      if (type === "water")
        console.log(
          `    pH: ${C.cyan}${d.ph?.toFixed(2) || "—"}${C.reset} | TDS: ${d.tds?.toFixed(1)} | Turbidity: ${d.turbidity?.toFixed(2)} | DO: ${d.dissolvedOxygen?.toFixed(2)} | BOD: ${d.bod?.toFixed(1)}`,
        );
      if (type === "noise")
        console.log(
          `    Laeq: ${C.magenta}${d.laeq?.toFixed(1) || "—"}${C.reset} dB(A) | Lmax: ${d.lmax?.toFixed(1)} | Lmin: ${d.lmin?.toFixed(1)}`,
        );
    });
...
async function submitData(type) {
  ...
  const locRes = await api(
    "GET",
    `/monitoring-locations?type=${type.toUpperCase()}`,
  );
  if (!locRes.ok) {
    err("Failed to fetch locations");
    await press();
    return;
  }
  printTable(locRes.data, [
  ...
  if (res.ok) {
    ...
    if (res.data.alertsGenerated > 0) {
      console.log(
        `  ${C.red}🚨 ${res.data.alertsGenerated} alert(s) automatically generated!${C.reset}`,
      );
...
async function manageAlerts() {
  ...
  res.data.forEach((a, i) => {
    ...
    console.log(
      `       Location: ${a.location?.name}  |  Status: ${scolor}${a.status}${C.reset}  |  ${new Date(a.createdAt).toLocaleString()}`,
    );
  });
  ...
  if (role === "SUPER_ADMIN" || role === "REGIONAL_OFFICER") {
    ...
    info(
      "Only SUPER_ADMIN or REGIONAL_OFFICER can acknowledge/resolve alerts.",
    );
  await press();
}
  ...
  await menu("REGIONAL OFFICES", [
    ...
    [
      ...
      async () => {
        ...
        const res = await api("POST", "/regional-offices", {
          name,
          code,
          district,
          address,
  ...
  await menu("INDUSTRIES", [
    ...
    [
      ...
      async () => {
        ...
        const res = await api("PUT", `/industries/${item.id}`, {
          status: status.trim(),
        });
        printResult(res);
        await press();
  ...
  await menu("WATER SOURCES", [
    ...
    [
      ...
      async () => {
        ...
        const lat = await ask("  Latitude: ");
        const lng = await ask("  Longitude: ");
        const res = await api("POST", "/water-sources", {
          name,
          type,
          lat: parseFloat(lat),
          lng: parseFloat(lng),
  ...
  await menu("MONITORING LOCATIONS", [
    ...
    [
      ...
      async () => {
        ...
        const res = await api(
          "GET",
          `/monitoring-locations?type=${type.trim().toUpperCase()}`,
        );
        if (res.ok)
          printTable(res.data, [
            ["name", "Name"],
    ...
    [
      ...
      async () => {
        ...
        const lat = await ask("  Latitude: ");
        const lng = await ask("  Longitude: ");
        const res = await api("POST", "/monitoring-locations", {
          name,
...
async function viewPrescribedLimits() {
  ...
  if (res.ok) {
    ...
    res.data.forEach((l, i) => {
      console.log(
        `      Unit: ${l.unit?.name} (${l.unit?.symbol})  |  Type: ${l.unit?.parameterType}`,
      );
    });
  } else err(res.data?.error);
  await press();
