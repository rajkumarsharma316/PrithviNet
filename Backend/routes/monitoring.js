...
export default async function monitoringRoutes(fastify, opts) {
  ...
  // POST — submit air quality data (Monitoring Team only)
  fastify.post('/air', {
    preHandler: [fastify.authenticate, authorize('MONITORING_TEAM')],
    schema: {
      body: {
        type: 'object',
        required: ['locationId'],
        properties: {
          ...
          pm25: { type: 'number' }, pm10: { type: 'number' },
          no2: { type: 'number' }, so2: { type: 'number' },
          co: { type: 'number' }, o3: { type: 'number' },
          aqi: { type: 'number' },
  ...
  fastify.post('/water', {
    ...
    schema: {
      body: {
        ...
        properties: {
          locationId: { type: 'string' },
          ph: { type: 'number' }, tds: { type: 'number' },
          turbidity: { type: 'number' }, dissolvedOxygen: { type: 'number' },
          bod: { type: 'number' }, cod: { type: 'number' },
          timestamp: { type: 'string', format: 'date-time' }
        }
      }
    }
    ...
    const alerts = await checkAndCreateAlerts(fastify.prisma, {
      type: 'WATER', locationId, parameters
    });
  ...
  fastify.post('/noise', {
    ...
    schema: {
      body: {
        ...
        properties: {
          locationId: { type: 'string' },
          laeq: { type: 'number' }, lmax: { type: 'number' }, lmin: { type: 'number' },
          timestamp: { type: 'string', format: 'date-time' }
        }
      }
    }
    ...
    const alerts = await checkAndCreateAlerts(fastify.prisma, {
      type: 'NOISE', locationId, parameters
    });
