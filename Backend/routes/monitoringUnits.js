...
export default async function monitoringUnitRoutes(fastify, opts) {
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const units = await fastify.prisma.monitoringUnit.findMany({
        where,
        orderBy: { name: "asc" },
      });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      const unit = await fastify.prisma.monitoringUnit.create({
        data: request.body,
      });
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      await fastify.prisma.monitoringUnit.delete({
