...
export default async function prescribedLimitRoutes(fastify, opts) {
  ...
  fastify.get('/', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const limits = await fastify.prisma.prescribedLimit.findMany({
      where,
      include: { unit: { select: { name: true, symbol: true, parameterType: true } } },
  ...
  fastify.post('/', {
    ...
    const limit = await fastify.prisma.prescribedLimit.create({ data: request.body });
  ...
  fastify.delete('/:id', {
    ...
    await fastify.prisma.prescribedLimit.delete({ where: { id: request.params.id } });
