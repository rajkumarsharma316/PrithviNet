...
export default async function waterSourceRoutes(fastify, opts) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!source)
        return reply.code(404).send({ error: "Water source not found" });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      const source = await fastify.prisma.waterSource.create({
        data: request.body,
      });
  ...
  fastify.put(
    ...
    async (request, reply) => {
      const source = await fastify.prisma.waterSource.update({
        where: { id: request.params.id },
        data: request.body,
      });
      return reply.send(source);
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      await fastify.prisma.waterSource.delete({
        where: { id: request.params.id },
      });
