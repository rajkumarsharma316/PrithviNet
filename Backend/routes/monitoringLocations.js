...
export default async function monitoringLocationRoutes(fastify, opts) {
  ...
  fastify.get('/', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const locations = await fastify.prisma.monitoringLocation.findMany({
      where,
      include: { region: { select: { name: true } }, industry: { select: { name: true } } },
      orderBy: { name: 'asc' }
    });
  ...
  fastify.get('/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    if (!location) return reply.code(404).send({ error: 'Monitoring location not found' });
  ...
  fastify.post('/', {
    ...
    const location = await fastify.prisma.monitoringLocation.create({ data: request.body });
  ...
  fastify.put('/:id', {
    ...
    const location = await fastify.prisma.monitoringLocation.update({ where: { id: request.params.id }, data: request.body });
    return reply.send(location);
  });
  ...
  fastify.delete('/:id', {
    ...
    await fastify.prisma.monitoringLocation.delete({ where: { id: request.params.id } });
