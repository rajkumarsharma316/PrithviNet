...
import { Wind, Droplets, Volume2, ArrowUpRight, ArrowDownRight, BarChart3, MapPin, Factory, AlertCircle, ChevronDown, CheckCircle, XCircle, Clock } from 'lucide-react';
import { getOverview, getMonitoringLocations } from '../api';
import { REGIONS, REGION_DETAILS, MOCK_ALERTS } from '../mockData';
...
const MonitoringCards = () => {
  ...
  useEffect(() => {
    getOverview().then(res => {
      if (res.ok) setOverview(res.data);
        ...
        setOverview({
          stats: { totalLocations: 8, totalIndustries: 142, totalRegions: REGIONS.length, activeAlerts: MOCK_ALERTS.length },
          latestReadings: {
  ...
  const totalIndustries = REGIONS.reduce((s, r) => s + (REGION_DETAILS[r.id]?.industries?.total || 0), 0);
  const compliantIndustries = REGIONS.reduce((s, r) => s + (REGION_DETAILS[r.id]?.industries?.compliant || 0), 0);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', width: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div className="glass-panel" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 16px', borderRadius: '12px', cursor: 'pointer' }}>
          ...
          <select
            ...
            style={{
              ...
              fontFamily: 'var(--font-body)', fontSize: '0.9rem', fontWeight: 500, outline: 'none',
              cursor: 'pointer', appearance: 'none', paddingRight: '20px'
            }}
          >
            <option value="" style={{ background: '#0f1523' }}>Chhattisgarh (State Overview)</option>
            {locations.map(l => (
              <option key={l.id} value={l.id} style={{ background: '#0f1523' }}>{l.name}</option>
            ))}
          </select>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
        <StatCard icon={<MapPin size={20} />} label="Monitoring Locations" value={stats?.totalLocations ?? '—'} color="#10b981" loading={!overview} onClick={() => navigate('/monitoring-locations')} />
        <StatCard icon={<Factory size={20} />} label="Active Industries" value={stats?.totalIndustries ?? '—'} color="#3b82f6" loading={!overview} onClick={() => navigate('/industries-list')} />
        <StatCard icon={<BarChart3 size={20} />} label="Regional Offices" value={stats?.totalRegions ?? '—'} color="#f59e0b" loading={!overview} onClick={() => navigate('/offices-list')} />
        <StatCard icon={<AlertCircle size={20} />} label="Active Alerts" value={stats?.activeAlerts ?? '—'} color="#ef4444" loading={!overview} onClick={() => navigate('/alerts-list')} />
      </div>

      {/* Compliance Summary */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
        <div className="glass-panel" style={{ padding: '20px' }}>
          <h3 style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '14px', fontWeight: 500 }}>Compliance Overview</h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '14px' }}>
            <div style={{ position: 'relative', width: '64px', height: '64px' }}>
              ...
              <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 700 }}>
                {totalIndustries > 0 ? `${Math.round(compliantIndustries / totalIndustries * 100)}%` : '—'}
              </span>
            </div>
            <div>
              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', margin: '0 0 4px' }}>State-wide compliance rate</p>
              <div style={{ display: 'flex', gap: '12px', fontSize: '0.8rem' }}>
                <span style={{ color: '#10b981', display: 'flex', alignItems: 'center', gap: '4px' }}><CheckCircle size={14} />{compliantIndustries}</span>
                <span style={{ color: '#ef4444', display: 'flex', alignItems: 'center', gap: '4px' }}><XCircle size={14} />{nonCompliantIndustries}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '20px' }}>
          <h3 style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '14px', fontWeight: 500 }}>Alert Summary</h3>
          <div style={{ display: 'flex', gap: '16px' }}>
            <AlertBubble label="Critical" count={criticalAlerts} color="#ef4444" />
            <AlertBubble label="Warning" count={warningAlerts} color="#fbbf24" />
            <AlertBubble label="Info" count={MOCK_ALERTS.filter(a => a.severity === 'INFO').length} color="#3b82f6" />
          </div>
        </div>

        <div className="glass-panel" style={{ padding: '20px' }}>
          <h3 style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '14px', fontWeight: 500 }}>Quick Stats</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
        ...
        <Card
          ...
          params={[
            ...
            { label: 'PM10', value: latest?.air?.pm10 ? `${latest.air.pm10.toFixed(1)} µg/m³` : '—', color: '#fbbf24' },
            ...
            { label: 'CO', value: latest?.air?.co ? `${latest.air.co.toFixed(1)} mg/m³` : '—', color: '#10b981' },
            { label: 'O₃', value: latest?.air?.o3 ? `${latest.air.o3.toFixed(1)} ppb` : '—', color: '#3b82f6' },
          ]}
          loading={!overview}
        />
        ...
        <Card
          ...
          params={[
            ...
            { label: 'BOD', value: latest?.water?.bod ? `${latest.water.bod.toFixed(1)} mg/L` : '—', color: '#fbbf24' },
            { label: 'COD', value: latest?.water?.cod ? `${latest.water.cod.toFixed(1)} mg/L` : '—', color: '#fbbf24' },
        ...
        <Card
          ...
          params={[
            { label: 'LAeq', value: latest?.noise?.laeq ? `${latest.noise.laeq.toFixed(1)} dB(A)` : '—', color: '#fbbf24' },
            { label: 'Peak (Lmax)', value: latest?.noise?.lmax ? `${latest.noise.lmax.toFixed(1)} dB(A)` : '—', color: '#ef4444' },
            { label: 'Min (Lmin)', value: latest?.noise?.lmin ? `${latest.noise.lmin.toFixed(1)} dB(A)` : '—', color: '#10b981' },
          ]}
          loading={!overview}
        />
      </div>
...
const StatCard = ({ icon, label, value, color, loading, onClick }) => (
  <div className="glass-panel" onClick={onClick} style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px', cursor: onClick ? 'pointer' : 'default', transition: 'all 0.2s' }}>
...
const AlertBubble = ({ label, count, color }) => (
  <div style={{ flex: 1, textAlign: 'center', padding: '12px 8px', borderRadius: '10px', background: `${color}08`, border: `1px solid ${color}20` }}>
    <p style={{ fontSize: '1.4rem', fontWeight: 700, color, margin: '0 0 2px', fontFamily: 'var(--font-display)' }}>{count}</p>
