...
const generateForecastData = () => {
  ...
  for (let i = 0; i < 72; i += 4) { // 72 hours forecast, every 4 hours
    ...
    <div className="glass-panel" style={{ padding: '24px', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div className="glass-panel" style={{ padding: '4px', borderRadius: '12px', display: 'flex', gap: '4px', background: 'rgba(255,255,255,0.05)' }}>
          <button 
             ...
             style={{ padding: '6px 12px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 600, color: activeParam === 'air' ? 'white' : 'var(--text-secondary)', background: activeParam === 'air' ? '#10b981' : 'transparent' }}>
             AQI (Air)
          </button>
          <button 
             onClick={() => setActiveParam('water')}
             style={{ padding: '6px 12px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 600, color: activeParam === 'water' ? 'white' : 'var(--text-secondary)', background: activeParam === 'water' ? '#3b82f6' : 'transparent' }}>
             pH (Water)
          </button>
          <button 
             onClick={() => setActiveParam('noise')}
             style={{ padding: '6px 12px', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 600, color: activeParam === 'noise' ? 'white' : 'var(--text-secondary)', background: activeParam === 'noise' ? '#ef4444' : 'transparent' }}>
             dB(A) (Noise)
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, minHeight: '250px' }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={forecastData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <ReferenceLine y={100} stroke="#f59e0b" strokeDasharray="3 3" opacity={0.5} label={{ position: 'insideTopLeft', value: 'Safe Limit', fill: '#f59e0b', fontSize: 10 }} />
      ...
      <div style={{ marginTop: '16px', display: 'flex', gap: '16px', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div style={{ width: '12px', height: '2px', background: '#10b981' }}></div> Point Forecast
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="glass-panel" style={{ padding: '12px', background: 'rgba(10,15,25,0.9)', border: '1px solid rgba(255,255,255,0.2)' }}>
        ...
        <p style={{ margin: '0 0 4px 0', fontSize: '1rem', fontWeight: 600, color: '#10b981' }}>
          Forecast: {aqi} AQI
        </p>
