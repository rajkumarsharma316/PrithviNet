...
export default function AdminLimitsPage() {
  ...
  useEffect(() => {
    getPrescribedLimits().then(r => { if (r.ok) setLimits(r.data); setLoading(false); });
  }, []);
      ...
      {loading ? <div className="page-loading"><span className="spinner"></span></div> : (
        <div className="admin-table-wrap glass-panel">
          <table className="admin-table">
            <thead><tr><th>Parameter</th><th>Min</th><th>Max</th><th>Category</th><th>Unit</th><th>Type</th></tr></thead>
            <tbody>
              {limits.map((l, i) => (
                <tr key={l.id || i}>
                  ...
                  <td>{l.maxValue ?? '—'}</td>
                  <td>{l.category || '—'}</td>
                  <td><span className="code-badge">{l.unit?.name} ({l.unit?.symbol})</span></td>
                  <td>{l.unit?.parameterType || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {limits.length === 0 && <p style={{ textAlign: 'center', padding: '24px', color: 'var(--text-muted)' }}>No limits configured</p>}
        </div>
      )}
    </div>
  );
